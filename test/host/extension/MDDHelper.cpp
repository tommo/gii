...
void fillTiles( USLeanArray <u32>& buffer, int w, int h, int x, int y, u32 newColor)
{
...
int MDDHelper::_fillGrid(lua_State *L){
	...
	MOAIGrid *grid=state.GetLuaObject<MOAIGrid>(1,true);
	USLeanArray<u32>& buffer=grid->GetTileArray();
...
int MDDHelper::_distanceBetweenTransform(lua_State *L){
	...
	if( !t1 || !t2 ) return 0;
	USVec3D v1=t1->GetLoc();
	USVec3D v2=t2->GetLoc();
