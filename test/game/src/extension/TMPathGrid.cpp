...
void quickFill(USLeanArray<u32>& buffer, int w, int h,	//buffer
								int pw, int ph,													//partition
								int fx, int fy, TMPathSection* section, //target
								TMPathSectionList& sections, u32 & seq, USLeanArray<u32>& seqs);
...
TMPathSection* TMPathGrid::setTile(u32 x, u32 y, u32 code){
	...
	USLeanArray <u32> & tiles=mSectionGrid->GetTileArray();
...
void TMPathGrid::init(u32 width, u32 height, float tileWidth, float tileHeight, u32 partitionWidth, u32 partitionHeight){
	...
	mSectionGrid->Init(width,height,1.0f,1.0f);
	mSectionGrid->GetTileArray().Init(mSectionGrid->GetTotalCells());
	mSectionGrid->Fill(0);

	mCodeGrid->Init(width,height,1.0f,1.0f);
...
// inline getTile(USLeanArray<u32> &buf, fx,fy)
...
void quickFill(USLeanArray<u32>& buffer, int w, int h,	//buffer
								int pw, int ph,													//partition
								int fx, int fy, TMPathSection* section, //target
								TMPathSectionList& sections, u32 & seq, USLeanArray<u32>& seqs){ 			//for avoid repeative checking
