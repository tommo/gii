...
void TMInfluenceMap::updateDiffusion ( u32 iteration ){
	UNUSED( iteration );	
	USLeanArray< float > *mapRead;
	USLeanArray< float > *mapWrite;
#if INFLUENCEMAP_USE_DOUBLE_BUFFER
	if( mCurrentMapId == 0 ){ 
		mapRead  = &mMap0;
		mapWrite = &mMap1;
...
void TMInfluenceMap::addCell ( u32 x, u32 y, float value) {
	USLeanArray< float > * map = getCurrentMap();
...
int TMInfluenceMapWalker::_getVector ( lua_State *L ) {
	...
	u32 x = state.GetValue< u32 >( 2, 1 ) - 1 ;
	u32 y = state.GetValue< u32 >( 3, 1 ) - 1 ;
	USIntVec2D vec = self->getVector( x, y );
	state.Push( vec.mX );
	state.Push( vec.mY );
	return 2;
}
...
void TMInfluenceMapWalker::updateStep () {
	USIntVec2D vec = getVector( mX, mY );
	mDx = vec.mX;
	mDy = vec.mY;
...
USIntVec2D TMInfluenceMapWalker::getVector ( u32 x0, u32 y0 ) {
	u32 x1 = x0 ;
	u32 y1 = y0 ;
