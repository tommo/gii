...
int MDDMapObject::_setCollisionListener ( lua_State *L ) {
	MOAI_LUA_SETUP( MDDMapObject, "UF" )
	self->SetLocal ( state, 2, self->mOnObjectCollision );
	self->SetLocal ( state, 3, self->mOnTileCollision );
	return 0;
}
...
bool MDDMapObject::computeTarget ( float delta ) {
	...
	if( !mTarget ){
		...
		USVec2D *p = new USVec2D( wp.mX, wp.mY );
		mTarget = p; 
	}

	if( !isRunningAway() ) {
		mVectorTarget = *mTarget - USVec2D( mLoc.mX, mLoc.mY );
	} else {
		mVectorTarget = USVec2D( mLoc.mX, mLoc.mY ) - *mTarget;
	}
	...
	if( length <= max( delta * mSpeed, mRadiusArrival ) ){
		...
		// mLoc.mX = mTarget->mX;
		// mLoc.mY = mTarget->mY;
		mVectorTarget = USVec2D( 0.0f, 0.0f );		
		clearTarget();
	} else {
		mVectorTarget.Norm();		
	}
...
int MDDMapObject::computeObjectCollision () {
	...
	if( count ){
		...
		for ( ; it != list.end (); ++it ) {
			...
			USVec2D diff ( x - o->mLoc.mX,	y - o->mLoc.mY );
			...
			if ( interception > 0.0f ) { //collided
				...
				if( this->mOnObjectCollision ) {
					this->PushLocal ( state, this->mOnObjectCollision );
...
int MDDMapObject::computeWallCollision () {
	...
	if ( collided ) {
		...
		if( this->mOnTileCollision ) {
			this->PushLocal ( state, this->mOnTileCollision );
			this->PushLuaUserdata( state );
			state.DebugCall ( 1, 1 );
...
//----------------------------------------------------------------//
void MDDMapObject::pushWayPoint ( USVec2D wayPoint ) {
...
//----------------------------------------------------------------//
void MDDMapObject::setTarget ( const USVec2D& target ) {	
	mTarget = new USVec2D( target.mX, target.mY );
}
...
void MDDMapObject::clearTarget() {
	...
	delete mTarget;
	mTarget = NULL;
	mVectorTarget = USVec2D( 0, 0 );
	setRunningAway( false );
}

bool MDDMapObject::moveTowardObject( MDDMapObject& obj, float chaseRadius ) {
	USVec3D loc1 = obj.GetLoc();
	USVec3D diff = loc1 - mLoc;
	float distance = diff.Length();
	if ( distance <= chaseRadius ) {
		clearTarget();
		return true;
	}
	setTarget( USVec2D( loc1.mX, loc1.mY ) );
	return false;
}
...
void MDDMapObject::moveToward( USVec2D vec ) {
	clearTarget();
	mVectorTarget = vec;
}
