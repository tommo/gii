...
class AnimProject(object):
	...
	def generateAtlases( self, prefix, size, **kwargs ):
	...
	def saveAurora( self, path, size, **kwargs ):
		...
		if not self.atlases:
			self.generateAtlases( prefix, size, **kwargs )
	...
	def getModule( self, psdLayer ):
		...
		if m: return m
		for l1, m1 in self.moduleCache.iteritems():
...
proj.loadPSD( 'watch.psd' )
proj.loadPSD( 'die.psd' )
proj.loadPSD( 'walk.psd' )

proj.saveAurora( 'explorer', ( 120, 120 ) )
