import subprocess
from moei.base import *
import os.path
import shutil
import time
import json

from moei.editor.controls import GLWidget
from moei.editor.propertyGrid import PropertyGrid
from moei.moai import MOAIEditCanvas
...
class MOEITexturePacker( EditorModule ):
	"""docstring for MOEITexturePacker"""
	def __init__(self):
		super(MOEITexturePacker, self).__init__()
		self.textureSet     = {}
		self.editingAsset   = None
		self.generatedAtlas = None
	
	def getName(self):
		return 'moai.texture_packer'
	...
	def onLoad(self):
		...
		self.container=main.requestDockWindow('MOAITexturePacker',
		...
		self.window = window = self.container.addWidgetFromFile(
				getProjectExtPath('common/texpackEditor.ui')
			)
		...
		self.previewCanvas.loadScript(getProjectExtPath('common/TexturePackerCanvas.lua'))
		...
		signals.connect('window.state.save', lambda:self.container.close() )
	...
	def generateAtlas(self):
		...
		arglist = ['python', getProjectExtPath('tools/atlasgenerator.py'), '--prefix', prefix, '1024', '1024']
		arglist += sourceList
		try:
...
registerModule(MOEITexturePacker())