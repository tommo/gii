...
--11/08/06 DCN Use _output not print
...
module ("clidebugger",package.seeall)
...
local _output=io.write
local _input=function()
  return io.read("*line")
end

local _send=function() end
...
local function indented( level, ... )
  _output( string.rep('  ',level), table.concat({...}), '\n' )
end

--}}}
--{{{  local function dumpval( level, name, value, limit )
local function sortvars(a,b)
...
local function _send_vars(v, scope, parent)
  local output={}
  local id=0
  ...
  if type(v)=='table' then
    for name,value in pairs(v) do
      ...
      if index then
        vtype,value=_valuetostring(value)
        id=id+1
        output[id]={type=vtype, name=index, value=value}
      end
    end
    table.sort(output,sortvars)
    _send('vars', {data=output, parent=parent or false, scope=scope})
  else
    print("????")
  end
end
...
local function _send_stack()
  local mark
  local list={}
  for level,ar in ipairs(traceinfo) do    
    local file=ar.source
    ...
    list[level]={
      ...
      line=ar.currentline,
      level=level,
      -- string='['..level..']'..'\t'..(ar.name or ar.what)..' in '..ar.short_src..':'..ar.currentline..'\n'
...
local function trace(set)
  ...
  for level,ar in ipairs(traceinfo) do
    ...
    _output('['..level..']'..mark..'\t'..(ar.name or ar.what)..' in '..ar.short_src..':'..ar.currentline..'\n')
  end
end
...
local function restore_vars(ref,vars)
  ...
  while true do
    ...
    if not name then break end
    if string.sub(name,1,1) ~= '(' and rawget(vars,name) then     --NB: ignoring internal control variables
  ...
  elseif event == "return" then
    ...
    while true do
      ...
      elseif tonumber(next) then --get vars for given level or last level
        ...
        _output('Unknown command from debugger_loop: '..tostring(next)..'\n')
        _output('Stopping debugger\n')
        next = 'stop'
      end
    end
  end
...
local function report(ev, vars, file, line, idx_watch)
  ...
  if ev == events.STEP then
    _output(prefix.."Paused at file "..file.." line "..line..' ('..stack_level[current_thread]..')\n')
  elseif ev == events.BREAK then
    _output(prefix.."Paused at file "..file.." line "..line..' ('..stack_level[current_thread]..') (breakpoint)\n')
  elseif ev == events.WATCH then
    _output(prefix.."Paused at file "..file.." line "..line..' ('..stack_level[current_thread]..')'.." (watch expression "..idx_watch.. ": ["..watches[idx_watch].exp.."])\n")
  elseif ev == events.SET then
    --do nothing
  else
    _output(prefix.."Error in application: "..file.." line "..line.."\n")
  end
  if ev ~= events.SET then
    if pausemsg and pausemsg ~= '' then _output('Message: '..pausemsg..'\n') end
    pausemsg = ''
    _send_stack()
  end
  return vars, file, line
  ...
  while true do
    ...
    if command == "setb" then
      ...
      if filename ~= '' and line ~= '' then
        ...
        _output("Breakpoint set in file "..filename..' line '..line..'\n')
      else
        _output("Bad request\n")
      end
    ...
    elseif command == "delb" then
      ...
      if filename ~= '' and line ~= '' then
        ...
        _output("Breakpoint deleted from file "..filename..' line '..line.."\n")
      else
        _output("Bad request\n")
      end
    ...
    elseif command == "delallb" then
      ...
      breakpoints = {}
      _output('All breakpoints deleted\n')
      --}}}
    ...
    elseif command == "setw" then
      ...
      if args and args ~= '' then
        ...
        _output("Set watch exp no. " .. newidx..'\n')
      else
        _output("Bad request\n")
      end
    ...
    elseif command == "delw" then
      ...
      if index then
        watches[index] = nil
        _output("Watch expression deleted\n")
      else
        _output("Bad request\n")
      end
    ...
    elseif command == "delallw" then
      ...
      watches = {}
      _output('All watch expressions deleted\n')
      --}}}

    elseif command == "listw" then
      --{{{  list watch expressions
      for i, v in pairs(watches) do
        _output("Watch exp. " .. i .. ": " .. v.exp..'\n')
      end
      --}}}
    ...
    elseif command == "set" then
      ...
      if eval_env.__VARSLEVEL__ then
        _output('Level: '..eval_env.__VARSLEVEL__..'\n')
      else
        _output('No level set\n')
      end
      --}}}
    ...
    elseif command == "what" then
      ...
      if args and args ~= '' then
        ...
        if type(v) == 'function' then
          ...
          if def then
            _output(def.what..' in '..def.short_src..' '..def.linedefined..'..'..def.lastlinedefined..'\n')
          else
            _output('Cannot get info for '..v..'\n')
          end
        else
          _output(v..' is not a function\n')
        end
      else
        _output("Bad request\n")
      end
    ...
    elseif command == "pause" then
      --{{{  not allowed in here
      _output('pause() should only be used in the script you are debugging\n')
      --}}}
    ...
    elseif line ~= '' then
      ...
      if func == nil then                             --Michael.Bringmann@lsi.com
        _output("Compile error: "..line..'\n')
      elseif not ok then
        _output("Compile error: "..func..'\n')
      else
...
function setupIO(input,output,send)
  _input=input or function() return io.read("*line") end
  _output=output or print
  _send=send or function() end
end

function getCoroutine()
  return coro_debugger
end