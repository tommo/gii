...
function WaypointGraphGizmo:__init( graph )
	self.graph = graph
	self.transform = MOAITransform.new()
	inheritTransform( self.transform, graph._entity:getProp( 'physics' ) )
end
...
--------------------------------------------------------------------
--Install
mock.WaypointGraph.onBuildSelectedGizmo = function( self )
